...
class blockCtrl extends mainCtrl{
	...
	function refetch($cache){
	    self::$app->template()->clearFile($cache);
		//@unlink(W3CA_MASTER_PATH."data/cache/template/".$cache);
		echo "ok";
	}
	/**
	 * 更改界面模块的位置
	...
	function transposition(){
		...
		if($rs){
			if($_POST['file_var']){
                self::$app->template()->clearFile($_POST['file_var']);
			}
	...
	public function export()
	{
		...
		if(empty($_POST['file_var'])){
		    ...
		    $exp=new BlockExp(self::_pb());
            $exp->setTplExpDir(self::$app->template()->getExportDir());
	...
	function remove_from(){
        ...
        if(!$frame){
            ...
            if($_POST['page_file_var']){
                self::$app->template()->clearFile($_POST['page_file_var']);
            }
        ...
        if($info['id']&&$pageblock->saveBlock($info,$info['id'])){
            ...
            if(empty($_POST['page_file_var'])){
                ...
                foreach ($page_b  as $pg){
                    $f=W3CA_MASTER_PATH."data/cache/template/".$pg['file_id'].".php";
                    if(file_exists($f)){
                        @unlink($f);
                    }
                }
            }else {
                self::$app->template()->setPageBlockManager($pageblock);
                $data= array_merge($data,self::$app->template()->clearFile($_POST['page_file_var'],$_POST['zyqtpl']));
                return $this->_view_return(array("error"=>"0",
	...
	function prototype($bid=0,$appoint=""){
		...
		$html->url=self::$app->route("block/save_prototype");
		if($bid){//修改
        ...
        if(empty($_POST['tpl_type'])){
            if(empty($_POST['tpl_val'])){
                ...
                if($block->save()){
                    if($_POST['page_file_var']){
                        self::$app->template()->setPageBlockManager($pageblock);
                        $data= self::$app->template()->clearFile($_POST['page_file_var'],$_POST['zyqtpl']);
    ...
    function init($bid=0){
        ...
        $pageblock=self::_pb();
        $this->_assign("init_action",self::$app->route("block/init_action"));
	...
	function refresh($c){
        ...
        {
            self::$app->template()->clearTplCache("web/block_prototype", "B-".str_replace("\\","-",$k));
        }
        return $this->_json_return(0,"",$all_blocks);
	}
}
