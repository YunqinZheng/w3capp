...
<form action="{APP_PATH}mysql_install/database" method="post">
    <div class="setting-field">
        <p>Mysql Host</p>
        <label><span>数据库连接：</span></label><input type="text" required value="127.0.0.1" placeholder="数据库" name="db_host"/></div>
    <div class="setting-field">
        <p>Connect Port</p>
        <label><span>数据库端口：</span></label><input type="text" required value="3306" placeholder="数据库连接端口" name="db_port"/></div>
    <div class="setting-field">
        <p>Database Name</p>
        <label><span>数据库名：</span></label><input type="text" required placeholder="数据库名需要手动创建" name="db_name"/></div>
    <div class="setting-field">
        <p>Database UserName</p>
        <label><span>数据库用户：</span></label><input type="text" required placeholder="数据库用户名" name="db_user"/></div>
    <div class="setting-field">
        <p>Database UserPassword</p>
        <label><span>数据库密码：</span></label><input type="text" name="db_pass"></div>
    <div class="setting-field">
        <p>TableName Prefix</p>
        <label><span>表前缀：</span></label><input type="text" value="w3ca_" name="db_table_pre"></div>
    <div class="setting-field">
        <p>Drop exists tables width same</p>
        <label><span>替换相同表：</span></label><input type="checkbox" checked value="1" name="drop_table">
    </div>
    <div class="form-footer">
		<img src="static/image/loading.gif" id="load" style="display:none;"/>
        <button type="button" onclick="$('#load').show();this.disabled=true;this.form.submit();" >安装/Install</button>
    </div>
</form>
</div>
</body>
