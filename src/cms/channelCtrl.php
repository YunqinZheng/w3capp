...
class channelCtrl extends mainCtrl{
		...
		foreach ($_POST['cid'] as $cid) {
			if($_POST['ch_name_'.$cid]){
				...
				if(array_key_exists('innav_'.$cid,$_POST)){
				    ...
				    if($data['innav']){
                        SiteNavigation::saveChannelNav($cid, $_POST['ch_name_'.$cid], self::$app->route($_POST['path_'.$cid]),1);
                    }else{
	...
	function add_channel(){
		...
		if($_POST['ch_name']){
			...
			if($channel->save()){
                $cid=$channel->primary();
				if($_POST['innav'])
                    SiteNavigation::saveChannelNav($cid, $_POST['ch_name'], self::$app->route($_POST['path']),$_POST['innav']);
                return $this->_referer_to("添加成功！",$_POST['sub_t']==1?self::$app->route("channel/index"):"","right");
			}else{
	...
	function edit_channel($id){
		...
		if($_POST['ch_name']){
			...
			if($cnnm->save()){
				if($_POST['innav']){
                    SiteNavigation::saveChannelNav($cnnm->id, $_POST['ch_name'], self::$app->route($_POST['path']),$_POST['innav']);
                }else{
                    SiteNavigation::deleteChannelNav($cnnm->id);
				}
				$this->_referer_to("修改成功！",self::$app->route("channel/index"),"right");
				return;
			}
	...
	function view_channel($id){
		...
		if(empty($cnnm)){
            ...
            return $this->_referer_to(null,($cnnm['static_path']?$cnnm['static_path']:self::$app->route($cnnm['path'])));
		}
	}
	function del_channel($ids){
		if(Channel::deleteAll(['id'=>explode(",",$ids)])){
            return $this->_referer_to("删除成功!",self::$app->route("channel"),"right");
		}else{
